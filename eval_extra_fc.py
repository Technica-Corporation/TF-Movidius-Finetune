...
tf.app.flags.DEFINE_string('split_name', 'validation', '')
tf.app.flags.DEFINE_integer('checkpoint_output_layers', 1000, 'number of output classes on final layer of checkpoint file')
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        # Select the model #
        ####################
        network_fn = nets_factory.get_network_fn(FLAGS.model_name, num_classes=FLAGS.checkpoint_output_layers, is_training=False)
        ...
        with tf.name_scope('final_training_ops'):
            with tf.name_scope('weights'):
                initial_value = tf.truncated_normal([FLAGS.checkpoint_output_layers, FLAGS.num_classes], stddev=0.001)
                layer_weights = tf.Variable(initial_value, name='final_weights')
            with tf.name_scope('biases'):
                layer_biases = tf.Variable(tf.zeros([FLAGS.num_classes]), name='final_biases')
            with tf.name_scope('mul'):
                final_tensor = tf.matmul(logits, layer_weights) + layer_biases
        softmax = tf.nn.softmax(final_tensor)
        variables_to_restore = slim.get_variables_to_restore()
        predictions = tf.argmax(final_tensor, 1)
        labels = tf.reshape(tf.squeeze(labels), (FLAGS.batch_size, ))
        # Define the metrics:
        names_to_values, names_to_updates = slim.metrics.aggregate_metric_map({'Accuracy': slim.metrics.streaming_accuracy(predictions, labels), 'Recall_5': slim.metrics.streaming_recall_at_k(softmax, labels, 5),})
